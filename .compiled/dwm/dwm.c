/* See LICENSE file for copyright and license details., Monitor *e, Monitor *e
 *
...
static void reesize(Client *c, int x, int y, int w, int h, int interact, Monitor *m);
static void resizeclient(Client *c, int x, int y, int w, int h, Monitor *m);
...
void
resize(Client *c, int x, int y, int w, int h, int interact, Monitor *m)
{
	if (applysizehints(c, &x, &y, &w, &h, interact))
		resizeclient(c, x, y, w, h, m);
}

void
resizeclient(Client *c, int x, int y, int w, int h, Monitor *m)
{
	XWindowChanges wc;
	...
	wc.border_width = c->bw;
	if (((nexttiled(c->mon->cl->clients, m) == c && !nexttiled(c->next, m))
...
sendmon(Client *c, Monitor *m)
{
	...
	attachstack(c);
	if (c->isfullscreen)
		resizeclient(c, m->mx, m->my, m->mw, m->mh, m);
	focus(NULL);
	arrange(NULL);
}
